...
import { ChangePasswordDto, AccountServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class ChangePasswordComponent extends AppComponentBase implements OnInit {
    ...
    public updatePassword(formValue: any) {
        ...
        this.isLoading = true;
        this.accountService.changePassword(changePasswordDto)
        .pipe(
            finalize(() => {
                this.doneLoading();
            })
        ...
        .subscribe(success => {
            if (success) {
                abp.message.success(this.l('PasswordChangedSuccessfully'), this.l('Success'));
